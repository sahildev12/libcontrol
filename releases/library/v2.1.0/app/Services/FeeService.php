...
class FeeService
{
    ...
    public function serializeRow(SeatBooking $booking): array
    {
        ...
        return [
            ...
            'joining_date' => $booking->joining_date?->format('M d'),
            'joining_date_iso' => $booking->joining_date?->toDateString(),
            'plan_expiry_date' => $feeType === 'one_time' ? null : $booking->plan_expiry_date?->format('M d'),
    ...
    public function suggestedRenewalStart(SeatBooking $booking): Carbon
    {
        $today = Carbon::today();
        $currentExpiry = $booking->plan_expiry_date;
        $start = $currentExpiry?->copy()->addDay() ?? $today->copy();
     ...
     */
    public function renewPlan(SeatBooking $booking, array $data): SeatBooking
    {
        $feeType = $this->normalizeFeeType((string) ($data['fee_type'] ?? $booking->fee_type));
        $paymentPlan = $this->normalizePaymentPlan($data['payment_plan'] ?? $booking->payment_plan, $feeType);
        $joining = isset($data['joining_date'])
            ? Carbon::parse($data['joining_date'])
            : $this->suggestedRenewalStart($booking);
        $expiry = $this->resolveExpiry(
            $feeType,
            $joining,
            isset($data['plan_expiry_date']) ? Carbon::parse($data['plan_expiry_date']) : null,
        );
        $frequency = $paymentPlan === 'installments'
            ? $this->normalizeFrequency((string) ($data['installment_frequency'] ?? $booking->installment_frequency ?? 'monthly'))
            : null;
        ...
        $booking->update([
            ...
            'payment_plan' => $paymentPlan,
            'installment_frequency' => $frequency,
            'fee_amount' => round((float) ($data['fee_amount'] ?? $booking->fee_amount), 2),
            'amount_paid' => 0,
            'fee_paid_at' => null,
            'joining_date' => $joining,
            'plan_expiry_date' => $expiry,
            'membership_mode' => $data['membership_mode'] ?? $booking->membership_mode,
            'expiry_reminder_sent_at' => null,
            'status' => $booking->status === 'on_trial' ? 'occupied' : $booking->status,
        ]);

        $booking = $booking->fresh();

        if ($paymentPlan === 'installments' && $frequency !== null) {
            $firstDue = isset($data['first_due_date'])
                ? Carbon::parse($data['first_due_date'])
                : $joining->copy();

            if ($frequency === 'custom') {
                $this->syncInstallments($booking, 0, 'custom', $firstDue);
            } else {
                $count = isset($data['installment_count'])
                    ? (int) $data['installment_count']
                    : $this->suggestedInstallmentCount($firstDue, $expiry, $frequency);

                $this->syncInstallments($booking, max(2, $count), $frequency, $firstDue);
            }
        }
        ...
        if ($paymentAmount > 0) {
            $booking = $this->recordPayment($booking->fresh('installments'), $paymentAmount, [
