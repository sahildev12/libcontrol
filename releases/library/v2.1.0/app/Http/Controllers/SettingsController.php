...
class SettingsController extends Controller
{
    ...
    public function index(Request $request, BranchBrandService $branchBrandService, AddonRegistry $addonRegistry, DatabaseMaintenanceService $databaseMaintenance): View
    {
        ...
        $addonCatalog = collect($addonRegistry->catalog())
            ->keys()
            ->map(fn (string $slug) => $addonRegistry->serializeForSettings($slug))
            ->values()
            ->all();

        return view('settings.index', compact('branch', 'settings', 'platformSettings', 'isPlatformAdmin', 'isDeveloperAdmin', 'planSnapshot', 'viewingAll', 'licenseServerEnabled', 'deploymentsUrl', 'addonCatalog', 'databaseMaintenance'));
    }
