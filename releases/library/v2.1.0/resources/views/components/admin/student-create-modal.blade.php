    ...
    <div class="relative flex max-h-[92vh] w-full max-w-3xl flex-col rounded-2xl bg-white shadow-2xl" @click.stop>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            ...
            <form id="seat-student-create-form" @submit.prevent="submitNewStudent()" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="sm:col-span-2 lg:col-span-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Contact &amp; Family</p>
                    <div class="mt-3 flex flex-wrap gap-4 text-sm">
                        <label class="inline-flex items-center gap-2">
                            <input type="radio" value="new" x-model="studentFamilyMode" @change="clearLinkedFamilyStudent()" class="text-indigo-600 focus:ring-indigo-500">
                            <span>New student</span>
                        </label>
                        <label class="inline-flex items-center gap-2">
                            <input type="radio" value="linked" x-model="studentFamilyMode" class="text-indigo-600 focus:ring-indigo-500">
                            <span>Link to existing family</span>
                        </label>
                    </div>

                    <div x-show="studentFamilyMode === 'linked'" x-cloak class="mt-3 space-y-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-700">Find sibling / family member</label>
                            <input
                                type="search"
                                x-model="familySearchQuery"
                                @input.debounce.300ms="searchFamilyStudents()"
                            ...
                            >
                            <p x-show="studentFormErrors.family" x-text="studentFormErrors.family" class="mt-1 text-xs text-red-600"></p>
                        </div>
                        <div x-show="familySearching" class="text-xs text-gray-500">Searching...</div>
                        <div x-show="familySearchResults.length > 0" class="max-h-36 overflow-y-auto rounded-lg border border-gray-200 bg-white">
                            <template x-for="result in familySearchResults" :key="result.id">
                                <button type="button" @click="selectLinkedFamilyStudent(result)" class="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-indigo-50">
                                    <span>
                                        <span class="font-medium text-gray-900" x-text="result.name"></span>
                                        <span class="text-gray-500" x-text="` · ${result.student_code}`"></span>
                                    </span>
                                    <span class="text-xs text-gray-500" x-text="result.phone || 'No contact'"></span>
                                </button>
                            </template>
                        </div>
                        <div x-show="linkedFamilyStudent" class="rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs text-indigo-900">
                            <p class="font-semibold">Shared family contact</p>
                            <p class="mt-1" x-text="`Linked with ${linkedFamilyStudent?.name} (${linkedFamilyStudent?.student_code})`"></p>
                            <p class="mt-1" x-text="`Phone: ${linkedFamilyStudent?.phone || '—'} · Email: ${linkedFamilyStudent?.email || '—'}`"></p>
                            <button type="button" @click="clearLinkedFamilyStudent()" class="mt-2 text-indigo-700 underline">Change selection</button>
                        </div>
                    </div>

                    <div x-show="studentFamilyMode === 'new' || (studentFamilyMode === 'linked' && contactRequiredForCreate())" class="mt-3 grid gap-3 sm:grid-cols-2">
                        <div>
                            <label class="block text-xs font-medium text-gray-700">
                                Phone
                                <span x-show="contactRequiredForCreate()" class="text-gray-500">(phone or email required)</span>
                                <span x-show="! contactRequiredForCreate()" class="text-gray-400">(optional)</span>
                            </label>
                        ...
                        <div>
                            ...
                            <input type="email" x-model="studentForm.email" class="mt-1 block w-full rounded-lg border px-3 py-1.5 text-sm" :class="studentFormErrors.email ? 'border-red-400' : 'border-gray-300'">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-700">Guardian / Father's Name</label>
