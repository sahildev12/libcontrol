...
<div x-show="editOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative flex max-h-[92vh] w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl" @click.stop>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-6 py-5">
            <div x-show="editForm.is_in_family" class="mb-4 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-900">
                <div class="flex flex-wrap items-center justify-between gap-2">
                    <div>
                        <p class="font-semibold">Family contact group</p>
                        <p class="text-xs" x-show="editForm.is_family_primary">You are editing the shared contact for this family.</p>
                        <p class="text-xs" x-show="! editForm.is_family_primary">Contact details are shared with linked siblings.</p>
                    </div>
                    <button type="button" @click="unlinkStudentFamily()" class="rounded-lg border border-indigo-300 bg-white px-3 py-1.5 text-xs font-semibold text-indigo-800 hover:bg-indigo-100">Unlink from family</button>
                </div>
                <div x-show="(editForm.linked_siblings || []).length > 0" class="mt-2 text-xs">
                    <span class="font-medium">Linked siblings:</span>
                    <template x-for="sibling in (editForm.linked_siblings || [])" :key="sibling.id">
                        <span class="ml-2" x-text="`${sibling.name} (${sibling.student_code})`"></span>
                    </template>
                </div>
            </div>
            ...
            <form id="student-edit-form" @submit.prevent="submitEditForm()" class="space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">
                            Contact
                            <span x-show="requireStudentContact && (! editForm.is_in_family || editForm.is_family_primary)" class="text-gray-500">(phone or email required)</span>
                            <span x-show="editForm.is_in_family && ! editForm.is_family_primary" class="text-gray-500">(shared)</span>
                        </label>
                        <input
                            type="tel"
                            ...
                            :readonly="editForm.is_in_family && ! editForm.is_family_primary"
                            class="mt-1 block w-full rounded-lg border px-3 py-2 text-sm shadow-sm disabled:bg-gray-100"
                ...
                <div class="grid gap-4 sm:grid-cols-2" x-show="! editForm.is_in_family || editForm.is_family_primary">
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Guardian / Father's Name</label>
                ...
                <div class="grid gap-4 sm:grid-cols-2" x-show="editForm.is_in_family && ! editForm.is_family_primary">
                    <div class="sm:col-span-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-600">
                        Phone and email are managed through the family's primary student contact.
                    </div>
                </div>
