...
function createFeeRenewMixin(options = {}) {
    ...
    return {
        ...
        canRenew(row) {
            return row && row.fee_type !== 'one_time' && row.plan_status !== 'cancelled';
        },
...
Alpine.data('settingsPage', (config) => ({
    ...
    updateAddonInCatalog(addon) {
        const index = this.addonCatalog.findIndex((item) => item.slug === addon.slug);
        if (index >= 0) {
            this.addonCatalog[index] = addon;
        }
    },
    ...
    async installAddon(slug) {
        ...
        try {
            ...
            this.updateAddonInCatalog(response.data.addon);
            showToast(response.data.message);
            window.setTimeout(() => window.location.reload(), 600);
        } catch (e) {
            showToast(extractAxiosError(e), 'error');
