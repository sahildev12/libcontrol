...
use App\Http\Requests\RenewFeeRequest;
...
class FeeController extends Controller
{
    ...
    public function renew(
        RenewFeeRequest $request,
        SeatBooking $booking,
        SeatMapService $seatMapService,
        FeeService $feeService,
    ): JsonResponse {
        $booking->load(['student', 'seat.hall.branch', 'installments']);
        $this->assertCanAccessBranch($request, $booking->seat?->hall?->branch_id);

        abort_if($booking->cancelled_at || $booking->status === 'cancelled', 422, 'Cannot renew a cancelled assignment.');
        abort_if(
            $feeService->normalizeFeeType((string) $booking->fee_type) === 'one_time',
            422,
            'One-time fees cannot be renewed. Set up a new fee plan instead.',
        );
        ...
        $booking = $feeService->renewPlan($booking, $payload);
        ...
        return response()->json([
            ...
            'row' => $feeService->serializeRow($booking),
        ]);
    }
