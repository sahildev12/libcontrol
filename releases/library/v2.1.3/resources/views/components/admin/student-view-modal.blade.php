...
<div x-show="viewOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative flex max-h-[92vh] w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl" @click.stop>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-6 py-5">
            ...
            <div x-show="! viewLoading && viewStudent" class="space-y-5">
                <div class="flex items-center gap-2">
                    ...
                    <span x-show="viewStudent?.is_in_family" class="inline-flex rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-800">Family</span>
                </div>

                <div x-show="(viewStudent?.linked_siblings || []).length > 0" class="rounded-lg border border-indigo-100 bg-indigo-50 px-4 py-3 text-sm text-indigo-900">
                    <p class="text-xs font-semibold uppercase tracking-wide text-indigo-700">Linked Siblings</p>
                    <template x-for="sibling in (viewStudent?.linked_siblings || [])" :key="sibling.id">
                        <p class="mt-1" x-text="`${sibling.name} (${sibling.student_code})`"></p>
                    </template>
                </div>
        ...
        <div class="flex shrink-0 justify-end gap-2 border-t border-gray-200 px-6 py-4">
            ...
            <button
                type="button"
                x-show="viewStudent && typeof openEditFromView === 'function'"
