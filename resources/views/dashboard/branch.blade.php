...
<div class="space-y-6">
    ...
    {{-- KPI cards: match main dashboard sizing --}}
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-start">
        <section class="min-w-0 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm {{ config('libcontrol.modules.enquiries') ? 'md:w-[58%] lg:w-[60%]' : 'w-full' }}">
            ...
            <div class="lc-kpi-row lc-kpi-row--lg grid grid-cols-1 gap-4 p-5 sm:grid-cols-2 {{ config('libcontrol.modules.enquiries') ? 'lg:grid-cols-2' : 'lg:grid-cols-3' }}">
                @if (config('libcontrol.modules.enquiries'))
                <x-admin.kpi-card label="New Enquiries" :value="number_format($today['enquiries'])">
                    <x-slot:icon>
                ...
                @endif
                <x-admin.kpi-card label="New Students" :value="number_format($today['students'])">
                    <x-slot:icon>
                ...
                <x-admin.kpi-card label="Today's Revenue" :value="'₹'.number_format($today['revenue'])">
                    <x-slot:icon>
                ...
                <x-admin.kpi-card
                    label="Expiring Plans (Next 7 Days)"
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-stretch">
        <section class="min-w-0 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm md:w-1/2" x-data="feeRenewModal()">
                ...
                <table class="w-full min-w-[520px] text-left text-sm">
                    <thead class="border-b border-gray-200 bg-gray-50 text-xs font-semibold uppercase tracking-wide text-gray-500">
                        <tr>
                            ...
                            <th class="px-3 py-3">Days Left</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-gray-700">
                        @forelse ($branch['expiring_plans'] as $plan)
                            <tr class="hover:bg-indigo-50/40">
                                ...
                                <td class="px-5 py-3 text-right">
                                    <button
                                        type="button"
                                        @click="openRenewById({{ $plan['booking_id'] }})"
                                        class="inline-flex size-8 items-center justify-center rounded-lg border border-amber-200 bg-amber-50 text-amber-800 transition-colors hover:bg-amber-100"
                                        title="Renew plan"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-10 text-center text-gray-500">No plans expiring soon.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
